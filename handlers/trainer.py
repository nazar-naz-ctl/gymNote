...
# ── Допоміжні функції для налаштувань ─────────────

async def get_settings() -> dict:
    doc = await mongo_db["settings"].find_one({"_id": "settings"})
    return doc or {}
...
async def get_questions() -> list:
    doc = await mongo_db["settings"].find_one({"_id": "questions"})
    return doc.get("list", []) if doc else []

async def save_questions(questions: list) -> None:
    await mongo_db["settings"].update_one(
        {"_id": "questions"},
        {"$set": {"list": questions}},
        upsert=True
...
async def save_trainer_program(user_id: int, program: str) -> None:
    ...
    programs = user.get("trainer_programs", []) if user else []
    ...
    await update_user_field(user_id, "trainer_programs", programs)
...
async def view_program(callback: CallbackQuery):
    ...
    if index >= len(programs):
        ...
        return
    programs[index]["read"] = True
    await update_user_field(callback.from_user.id, "trainer_programs", programs)
...
async def trainer_send_program(message: Message, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(user_id, "📬 <b>Тренер надіслав нову програму!</b>\n\nВідкрий: 📬 Від тренера")
...
async def trainer_broadcast_send(message: Message, state: FSMContext):
    ...
    target_labels = {"all": "всім", "premium": "Преміум", "standard": "Стандарт", "free": "Безкоштовним"}
...
@router.callback_query(F.data.startswith("t_sub_") & ~F.data.startswith("t_setsub_") & ~F.data.startswith(
    "t_set_sub_") & ~F.data.startswith("t_add_days_"))
...
async def trainer_settings(callback: CallbackQuery):
    ...
    requisites = settings.get("requisites", "Не вказано")
    prices = settings.get("prices", {"standard": "200 грн", "premium": "450 грн"})
    channel = settings.get("channel_link", "Не вказано")
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def trainer_set_requisites(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("💳 Введи нові реквізити:\nНаприклад: Monobank: 4441 1111 2222 3333")
...
async def trainer_set_prices(callback: CallbackQuery):
    ...
    await callback.message.edit_text("💰 <b>Ціни підписок</b>", reply_markup=kb)
...
async def save_price_standard(message: Message, state: FSMContext):
    ...
    await save_settings({"prices": prices})
    await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def save_price_premium(message: Message, state: FSMContext):
    ...
    settings = await get_settings()
    prices = settings.get("prices", {"standard": "200 грн", "premium": "450 грн"})
    prices["premium"] = message.text.strip()
    await save_settings({"prices": prices})
    await state.clear()
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="← Назад", callback_data="t_set_prices")],
...
async def trainer_set_channel(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📢 Введи посилання на канал:\nНаприклад: https://t.me/gymnote_news")
...
async def trainer_save_channel(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Посилання на канал збережено!", reply_markup=kb)
...
async def buy_premium(callback: CallbackQuery):
    settings = await get_settings()
    prices = settings.get("prices", {"premium": "450 грн"})
    requisites = settings.get("requisites", "Реквізити не вказані")
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def reject_payment(callback: CallbackQuery):
    ...
    try:
        from bot import bot
        await bot.send_message(user_id, "❌ <b>Оплату не підтверджено.</b>\n\nЗв'яжись з тренером для уточнення.")
    except Exception:
        pass
...
# ── Результати та рекорди клієнта (тренер) ────────
