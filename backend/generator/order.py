...
def _macro_tier(ex: dict) -> int:
    ...
    if ex_type in MACRO_TIER_BY_EX_TYPE:
        # Слот із шаблону дня (abs/calves включно) — ТВЕРДИЙ тир.
        # Властивість вправи 'type' з бази (здоров'я/розтяжка) більше
        # НЕ перекриває його: якщо тренер розмістив вправу в слоті
        # "прес" чи "литки", вона лишається в цьому тирі, навіть якщо
        # сама вправа за характером — дихальна/мобільна (напр. Вакуум
        # живота в слоті "прес" — це тир 3, а не тир 5).
        return MACRO_TIER_BY_EX_TYPE[ex_type]
    # MOBILITY_TIER — лише для вправ, які взагалі НЕ потрапили в
    # жоден твердий слот (ex_type відсутній/невідомий), але за
    # характером явно мобільність/розтяжка.
